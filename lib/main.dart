...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<TextEditingController> sizeController = [
    TextEditingController(text: double.infinity.toString()),
  ...
  void initState() {
    bluetooth = Bluetooth(
        onUpdate: _bleUpdate, names: ['P12Pro', 'D35', 'D30', 'Q155E2AQ1100400']

        /// 'Q--------------' Place the name of the m220 here. This is displayed on the front of the printer.
        );
  ...
  Future<void> printPhomemo(
      {Uint8List? image, String? name, Size? size}) async {
    size ??= const Size(double.infinity, 12);
    if (printing) return;
    printing = true;
    Phomemo label = Phomemo(send: bluetooth.write);
    PhomemoPrinter printer =
        PhomemoHelper.getPrinterFromName(bluetooth.device!.platformName);
    img.Image? letter;
    if (showType == 'Custom') {
    ...
    await label.printLabel([letter],
        printer: printer,
        spacing: 5,
        labelSize: labelSize,
        rotate: printer !=
            PhomemoPrinter
                .m220 // do not rotate the image if using the m220 or m110
        ).then((value) {
      printing = false;
    });
  }
  ...
  Future<img.Image?> textToImage(String? text, Size size) async {
    img.Image? letter = text != null
        ? await PhomemoHelper.generateImageFromText(
            TextSpan(
              text: text,
              style: const TextStyle(
                  fontFamily: 'MuseoSans', fontSize: 34, color: Colors.black),
            ),
            size: size * 8,
          )
        : null;
        ...
        child = Container(
          ...
          child: Row(
            children: [
              ...
              labelSize.width == double.infinity
                  ? const Text('This is an Example of the printer printing!')
                  : labelSize.width > 35
                      ? const Text('Example!')
                      : const Text('Ex!')
            ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          children: <Widget>[
            Text(
              bluetooth.deviceConnected()
                  ...
                  : 'Press floating action button to connect to a phomemo printer.',
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
              ...
              children: [
                ...
                Container(
                    ...
                    child: TextField(
                      ...
                      onChanged: (t) {
                        labelSize = Size(
                            sizeController[0].text != ''
                                ? double.parse(sizeController[0].text)
                                : double.infinity,
                            sizeController[1].text != ''
                                ? double.parse(sizeController[1].text)
                                : 12);
                        setState(() {});
                      },
                      decoration: const InputDecoration(
      ...
      floatingActionButton: FloatingActionButton(
          ...
          if (bluetooth.deviceConnected()) {
            labelSize = Size(
                ...
                sizeController[1].text != ''
                    ? double.parse(sizeController[1].text)
                    : 12);
            printPhomemo(name: textController.text, size: labelSize);
        ...
        child: Icon(bluetooth.deviceConnected()
            ? Icons.print
            : ((!bluetooth.isScanning && bluetooth.device == null)
