...
class Bluetooth {
  Bluetooth({BluetoothCallback? onUpdate, List<Guid>? services, this.names}) {
    guid = services ?? [];
    if (onUpdate != null) {
        ...
        if (allowScan && scanResult.isNotEmpty) {
          if (guid.isNotEmpty) {
            ...
            for (int i = 0; i < names!.length; i++) {
              if (scanResult[0].device.platformName.toLowerCase() ==
                  names![i].toLowerCase()) {
                scanResults = scanResult;
  ...
  void _connect(BluetoothDevice d) async {
    ...
    device!.connectionState.listen((onData) {
      if (onData == BluetoothConnectionState.disconnected &&
          deviceConnected()) {
